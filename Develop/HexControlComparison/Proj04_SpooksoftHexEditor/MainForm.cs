...
{
    // ════════════════════════════════════════════════════════════════
    //  Proj04: Spooksoft.HexEditor 1.0.3  (ElementHost 経由で WinForms に組み込み)
    //
    //  Tab②: DataGridViewSpooksoftHexColumn を使用。
    //         DataGridView の標準編集機構により HexEditorDisplay が
    //         そのままセル枠に合わせて表示される。
    //         HexEditorDisplay はクロム非表示設定がないため行高さを大きめに設定。
    //
    //  課題対応状況:
    //    ① 超過バイト数登録  … CellEndEdit でバイト数検証、不一致は破棄
    //    ② 不足バイト数登録  … 同上
    //    ③ 1バイト単位削除   … CellEndEdit でバイト数検証で検出
    //    ④ 横スクロール追従  … WPF レンダリングにより改善期待
    // ════════════════════════════════════════════════════════════════
    public class MainForm : Form
    {
        ...
        // HexEditorDisplay はクロム除去不可のため大きめに設定
        const int RowH = 80;
        ...
        public MainForm()
        {
            Text          = "④ Spooksoft.HexEditor 1.0.3";
        ...
        void BuildUI()
        {
            ...
            _status = new ToolStripStatusLabel("Spooksoft.HexEditor 1.0.3 (ElementHost)")
        ...
        TabPage BuildTab1()
        {
            ...
            _fullHex  = new HexEditorDisplay();
            _fullHost = new ElementHost { Dock = DockStyle.Fill, Child = _fullHex };

            var note = MakeNotePanel(
                "設定パラメータ: 調査中（Spooksoft.HexEditor 1.0.3 はドキュメントが少ない）\r\n" +
                "Tab②はコミット時にバイト数を検証し、不一致は変更を破棄します。");
        ...
        DataGridView BuildParamGrid()
        {
            ...
            gv.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(140, 70, 20);
            ...
            gv.Columns.Add(new DataGridViewSpooksoftHexColumn { Name = "HexBytes", HeaderText = "バイト値（HexEditorDisplay）", FillWeight = 52 });
        ...
        void LoadFullHex()
        {
            _fullHex.Document = new HexByteContainer(new MemoryStream(_data), 4096);
        }
        ...
        void SaveFile()
        {
            HexByteContainer doc = _fullHex.Document;
            if (doc != null) { _data = new byte[doc.Size]; for (int i = 0; i < doc.Size; i++) _data[i] = doc.GetByte(i); }
        ...
        {
            var p = new Panel { Dock = DockStyle.Bottom, Height = 44, BackColor = Color.FromArgb(255, 245, 235) };
