...
{
    // ════════════════════════════════════════════════════════════════
    //  Proj02: WPFHexaEditor 2.1.7  (ElementHost 経由で WinForms に組み込み)
    //
    //  Tab②: DataGridViewWpfHexColumn を使用。
    //         DataGridView の標準編集機構により HexEditor が
    //         そのままセル枠に合わせて表示される（Proj01 と同じ方式）。
    //
    //  課題対応状況:
    //    ① 超過バイト数登録  … AllowExtend=false で根本から禁止
    //    ② 不足バイト数登録  … AllowExtend=false + CellEndEdit でバイト数検証
    //    ③ 1バイト単位削除   … AllowExtend=false の効果次第（要確認）
    ...
    public class MainForm : Form
    {
        ...
        // HexEditor をセルに収めるための行の高さ（クロム除去済みのため 36px で十分）
        const int RowH = 36;
        ...
        void BuildUI()
        {
            ...
            _status = new ToolStripStatusLabel("WPFHexaEditor 2.1.7 (ElementHost)")
        ...
        DataGridView BuildParamGrid()
        {
            ...
            gv.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(50, 120, 50);
        ...
        {
            var p = new Panel { Dock = DockStyle.Bottom, Height = 44, BackColor = Color.FromArgb(240, 255, 240) };
