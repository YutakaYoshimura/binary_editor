...
namespace Be.Windows.Forms
{
	...
	public class HexBox : Control
	{
		...
		class KeyInterpreter : IKeyInterpreter
		{
			...
			{
				var provider = _hexBox._byteProvider as DynamicByteProvider;
				if (provider != null && provider.CanUndo)
				{
					provider.Undo();
					_hexBox.UpdateScrollSize();
					_hexBox.UpdateCaret();
			...
			{
				var provider = _hexBox._byteProvider as DynamicByteProvider;
				if (provider != null && provider.CanRedo)
				{
					provider.Redo();
					_hexBox.UpdateScrollSize();
					_hexBox.UpdateCaret();
			...
			public virtual bool PreProcessWmChar(ref Message m)
			{
				...
				if (Uri.IsHexDigit(c))
				{
					...
					// ===== 追加：課題② 行数制限チェック（挿入モードのみ制限） =====
					if (isInsertMode && _hexBox._maxLines > 0
						&& _hexBox._byteProvider.Length >= _hexBox.MaxByteCount)
					{
						return true; // 上限に達しているため入力を拒否
					}
					// ===== 追加ここまで =====
			...
			Dictionary<Keys, MessageDelegate> MessageHandlers
			{
				get
				{
					if (_messageHandlers == null)
					{
						...
						// ===== 追加：課題① Undo/Redo =====
						_messageHandlers.Add(Keys.Z | Keys.Control, new MessageDelegate(PreProcessWmKeyDown_ControlZ)); // undo
						_messageHandlers.Add(Keys.Y | Keys.Control, new MessageDelegate(PreProcessWmKeyDown_ControlY)); // redo
						// ===== 追加ここまで =====
					}
		...
		/// <summary>水平スクロール位置（文字単位）</summary>
		int _scrollHpos;
		// ===== 追加ここまで =====
		/// <summary>
		...
		// ===== 追加：課題② 行数制限 =====
		/// <summary>最大行数（0 = 無制限）</summary>
		int _maxLines = 0;
		// ===== 追加ここまで =====

		// ===== 追加：課題③ 最小バイト数 =====
		/// <summary>最小バイト数（0 = 制限なし）</summary>
		long _minByteLength = 0;
		// ===== 追加ここまで =====
		#endregion
		...
		public HexBox()
		{
			...
			this._hScrollBar.Scroll += new ScrollEventHandler(_hScrollBar_Scroll);
		...
		public bool CanPasteHex()
		{
			...
			if (da.GetDataPresent(typeof(string)))
			{
				...
				// ===== 修正：課題⑦ スペース区切り形式と連続16進数形式の両方を受け付ける =====
				byte[] buffer = ConvertHexToBytes(text);
				if (buffer != null) return true;
				buffer = ConvertContinuousHexToBytes(text);
				return (buffer != null);
				// ===== 修正ここまで =====
			}
			return false;
		...
		/// [修正⑦] スペース区切り「1A 2B」と連続形式「1A2B」→ 0x1A,0x2B の両方に対応。
		/// [修正②] MaxLines による行数制限を適用。
		/// </summary>
		public void PasteHex()
		{
			...
			if (da.GetDataPresent(typeof(string)))
			{
				...
				// ===== 修正：課題⑦ 貼り付け形式の自動判定 =====
				// まずスペース区切り形式（"1A 2B 3C"）として解釈を試みる
				if (IsSpaceSeparatedHexString(text))
					buffer = ConvertHexToBytes(text);

				// スペース区切りでなければ連続16進数形式（"1A2B" → 0x1A, 0x2B）として解釈
				if (buffer == null)
					buffer = ConvertContinuousHexToBytes(text);
			...
			// ===== 追加：課題② 行数制限チェック（貼り付け時） =====
			if (_maxLines > 0)
			{
				long currentLen = _byteProvider.Length - (_selectionLength > 0 ? _selectionLength : 0);
				long maxBytes = MaxByteCount;
				if (currentLen >= maxBytes) return; // 上限に達しているため拒否
				if (currentLen + buffer.Length > maxBytes)
				{
					// 入る分だけトリム
					int allowed = (int)(maxBytes - currentLen);
					byte[] trimmed = new byte[allowed];
					Array.Copy(buffer, trimmed, allowed);
					buffer = trimmed;
				}
			}
			// ===== 追加ここまで =====
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			// ===== 追加：課題⑥ カレント行ハイライト =====
			PaintCurrentLineHighlight(e.Graphics);
			// ===== 追加ここまで =====
		...
		// ===== 追加：課題⑥ カレント行ハイライト描画メソッド =====
		/// <summary>
		/// カレントバイト位置の行を CurrentLineColor でハイライト表示します。
		/// CurrentLineColor が Color.Empty の場合は描画しません。
		/// </summary>
		void PaintCurrentLineHighlight(Graphics g)
		{
			if (CurrentLineColor == Color.Empty) return;
			if (_byteProvider == null || _bytePos < 0) return;
			if (_charSize.Height <= 0 || _iHexMaxHBytes <= 0) return;

			// カレントバイトの行番号（表示上の相対行）を算出
			long currentLine = (_bytePos / _iHexMaxHBytes) - _scrollVpos;
			if (currentLine < 0 || currentLine >= _iHexMaxVBytes) return;

			float y = _recHex.Y + currentLine * _charSize.Height;
			RectangleF lineRect = new RectangleF(
				_recContent.X,
				y,
				_recContent.Width - (_vScrollBar.Visible ? _vScrollBar.Width : 0),
				_charSize.Height);

			using (SolidBrush brush = new SolidBrush(CurrentLineColor))
			{
		...
		// ===== 追加：課題⑦ 貼り付け形式判定・変換ヘルパー =====

		/// <summary>
		/// スペース区切りの16進文字列かどうかを判定します。
		/// 例：「1A 2B 3C」→ true、「1A2B3C」→ false
		/// </summary>
		bool IsSpaceSeparatedHexString(string s)
		{
		...
		/// <summary>
		/// 連続した16進文字列をバイト配列に変換します。
		/// 「1234」→ { 0x12, 0x34 }
		/// 16進文字以外は無視、奇数桁の場合は先頭に 0 を補完します。
		/// </summary>
		byte[] ConvertContinuousHexToBytes(string s)
		{
        ...
        // ===== 追加：課題② 行数制限プロパティ =====
        /// <summary>
        /// 入力可能な最大行数を取得または設定します。0 の場合は無制限です。
        /// BytesPerLine と組み合わせて使用してください。
        /// </summary>
        [DefaultValue(0), Category("Hex"),
         Description("Gets or sets the maximum number of lines. 0 means unlimited. Use with BytesPerLine.")]
        public int MaxLines
        {
        ...
        /// <summary>
        /// 現在設定されている最大バイト数を返します（MaxLines × BytesPerLine）。
        /// MaxLines が 0 の場合は long.MaxValue を返します。
        /// </summary>
        public long MaxByteCount
        {
            get
            {
                if (_maxLines <= 0) return long.MaxValue;
                return (long)_maxLines * _bytesPerLine;
            }
        }
        // ===== 追加ここまで =====
        ...
        // ===== 追加：課題③ 最小バイト数プロパティ =====
        /// <summary>
        /// 必要な最小バイト数を取得または設定します。0 の場合は制限なしです。
        /// 登録（保存）ボタン等で IsByteCountValid を確認してください。
        /// </summary>
        [DefaultValue(0L), Category("Hex"),
         Description("Gets or sets the minimum required byte length. 0 means no minimum.")]
        public long MinByteLength
        {
            get { return _minByteLength; }
            set { _minByteLength = value; }
        }

        /// <summary>
        /// 現在のバイト数が MinByteLength を満たしているかどうかを返します。
        /// </summary>
        public bool IsByteCountValid
        {
            get
            {
                if (_minByteLength <= 0) return true;
                if (_byteProvider == null) return _minByteLength == 0;
                return _byteProvider.Length >= _minByteLength;
            }
        }
        // ===== 追加ここまで =====

        // ===== 追加：課題⑥ カレント行ハイライト色プロパティ =====
        /// <summary>
        /// カレント行のハイライト色を取得または設定します。
        /// Color.Empty を設定するとハイライトを無効にします。
        /// </summary>
        [Category("Hex"),
         Description("Gets or sets the highlight color for the current line. Set to Color.Empty to disable.")]
        public Color CurrentLineColor { get; set; } = Color.FromArgb(30, 0, 120, 215);
        // ===== 追加ここまで =====
